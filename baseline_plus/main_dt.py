...
parser.add_argument("-o", "--output", help="output path:", type=str, default='../cpu_output/s120000r4_dt_plus/')
...
dtpredlist = np.loadtxt('/home/wamdm/xinli/competition/gwac_gpu/cpu_output/S120000R4/DT_finelpred_onid.csv',dtype=int, delimiter=",")
dtneg_predpositiveid = np.where(dtpredlist[0:3976] == 1)[0]
print("决策树本来预测错的正样本个数",len(dtneg_predpositiveid))
...
testindex = np.where(np.isin(equalseries_index_023, dtneg_predpositiveid) == True)[0]
# 加载测试数据集
...
def metrics(pred,model_name):
    ...
    for j, pindex in enumerate(equalseries_index):
        if pindex == dtneg_predpositiveid[flag]:
            finelpred[dtneg_predpositiveid[flag]] = finelpred[dtneg_predpositiveid[flag]] | pred[j]
        else:
            print(finelpred[dtneg_predpositiveid[flag]])
            flag = flag + 1
            finelpred[dtneg_predpositiveid[flag]] = finelpred[dtneg_predpositiveid[flag]] | pred[j]

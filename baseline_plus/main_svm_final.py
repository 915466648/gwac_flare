...
parser.add_argument("-o", "--output", help="output path:", type=str, default='../cpu_output/s120000r4_svm_plus/')
...
svmpredlist = np.loadtxt('/home/wamdm/xinli/competition/gwac_gpu/cpu_output/s120000r4/SVM_finelpred_onid.csv',dtype=int, delimiter=",")
svmneg_predpositiveid = np.where(svmpredlist[0:3976] == 1)[0]
print("svm本来预测错的正样本个数",len(svmneg_predpositiveid))
...
testindex = np.where(np.isin(equalseries_index_023, svmneg_predpositiveid) == True)[0]
...
def load_test_data(model_name):
    ...
    if model_name == 'KNN' or model_name == 'TCN' or model_name == 'FCN' or model_name == 'Resnet' or model_name == 'GRU' or model_name == 'BIGRU' or model_name == 'CNN1D' or model_name == 'all':
        ...
        negative_dataset = np.loadtxt('../dataset/svm_neg_scale_dataset.csv', dtype=float, delimiter=',')
        ...
        negative_dataset = np.loadtxt('../dataset/svm_neg_scale_dataset_feature.csv', dtype=float, delimiter=',')
...
def metrics(pred,model_name):
    equalseries_index = \
    np.loadtxt('../dataset/023_15730595-G0013_negative_test_equalseries_index.csv', dtype=int, delimiter=',')[testindex]
    ...
    for j, pindex in enumerate(equalseries_index):
        if pindex == svmneg_predpositiveid[flag]:
            finelpred[svmneg_predpositiveid[flag]] = finelpred[svmneg_predpositiveid[flag]] | pred[j]
        else:
            # print(finelpred[svmneg_predpositiveid[flag]])
            flag = flag + 1
            finelpred[svmneg_predpositiveid[flag]] = finelpred[svmneg_predpositiveid[flag]] | pred[j]
